...
public class LatifiCropOverlayView extends View {
    ...
    private final LatifiCropWindowHandler mLatifiCropWindowHandler = new LatifiCropWindowHandler();
    ...
    private LatifiCropWindowMoveHandler mMoveHandler;
    ...
    private LatifiCropImageView.Guidelines mGuidelines;

    /** The shape of the cropping area - rectangle/circular. */
    private LatifiCropImageView.CropShape mCropShape;
    ...
    public LatifiCropOverlayView(Context context) {
        this(context, null);
    }

    public LatifiCropOverlayView(Context context, AttributeSet attrs) {
        super(context, attrs);
    }
    ...
    public RectF getCropWindowRect() {
        return mLatifiCropWindowHandler.getRect();
    }

    /** Set the left/top/right/bottom coordinates of the crop window. */
    public void setCropWindowRect(RectF rect) {
        mLatifiCropWindowHandler.setRect(rect);
    }
    ...
    public void setBounds(float[] boundsPoints, int viewWidth, int viewHeight) {
        if (boundsPoints == null || !Arrays.equals(mBoundsPoints, boundsPoints)) {
            ...
            RectF cropRect = mLatifiCropWindowHandler.getRect();
    ...
    /** The shape of the cropping area - rectangle/circular. */
    public LatifiCropImageView.CropShape getCropShape() {
        return mCropShape;
    }

    /** The shape of the cropping area - rectangle/circular. */
    public void setCropShape(LatifiCropImageView.CropShape cropShape) {
        if (mCropShape != cropShape) {
            mCropShape = cropShape;
            if (Build.VERSION.SDK_INT <= 17) {
                if (mCropShape == LatifiCropImageView.CropShape.OVAL) {
    ...
    public LatifiCropImageView.Guidelines getGuidelines() {
        return mGuidelines;
    }
     ...
     * application.
     */
    public void setGuidelines(LatifiCropImageView.Guidelines guidelines) {
    ...
    public void setMinCropResultSize(int minCropResultWidth, int minCropResultHeight) {
        mLatifiCropWindowHandler.setMinCropResultSize(minCropResultWidth, minCropResultHeight);
    }
    ...
    public void setMaxCropResultSize(int maxCropResultWidth, int maxCropResultHeight) {
        mLatifiCropWindowHandler.setMaxCropResultSize(maxCropResultWidth, maxCropResultHeight);
    }
    ...
    public void setCropWindowLimits(
        ...
        mLatifiCropWindowHandler.setCropWindowLimits(
     ...
     */
    public void setInitialAttributeValues(LatifiCropImageOptions options) {

        mLatifiCropWindowHandler.setInitialAttributeValues(options);
    ...
    private void initCropWindow() {
        ...
        if (mInitialCropWindowRect.width() > 0 && mInitialCropWindowRect.height() > 0) {
            ...
            rect.left =
                    leftLimit + mInitialCropWindowRect.left / mLatifiCropWindowHandler.getScaleFactorWidth();
            rect.top = topLimit + mInitialCropWindowRect.top / mLatifiCropWindowHandler.getScaleFactorHeight();
            rect.right =
                    rect.left + mInitialCropWindowRect.width() / mLatifiCropWindowHandler.getScaleFactorWidth();
            rect.bottom =
                    rect.top + mInitialCropWindowRect.height() / mLatifiCropWindowHandler.getScaleFactorHeight();
            ...
            if (bitmapAspectRatio > mTargetAspectRatio) {
                ...
                float cropWidth =
                        Math.max(mLatifiCropWindowHandler.getMinCropWidth(), rect.height() * mTargetAspectRatio);
                ...
                // Limits the aspect ratio to no less than 40 wide or 40 tall
                float cropHeight =
                        Math.max(mLatifiCropWindowHandler.getMinCropHeight(), rect.width() / mTargetAspectRatio);
    ...
    private void fixCropWindowRectByRules(RectF rect) {
        if (rect.width() < mLatifiCropWindowHandler.getMinCropWidth()) {
            float adj = (mLatifiCropWindowHandler.getMinCropWidth() - rect.width()) / 2;
            rect.left -= adj;
            rect.right += adj;
        }
        if (rect.height() < mLatifiCropWindowHandler.getMinCropHeight()) {
            float adj = (mLatifiCropWindowHandler.getMinCropHeight() - rect.height()) / 2;
            rect.top -= adj;
            rect.bottom += adj;
        }
        if (rect.width() > mLatifiCropWindowHandler.getMaxCropWidth()) {
            float adj = (rect.width() - mLatifiCropWindowHandler.getMaxCropWidth()) / 2;
            rect.left += adj;
            rect.right -= adj;
        }
        if (rect.height() > mLatifiCropWindowHandler.getMaxCropHeight()) {
            float adj = (rect.height() - mLatifiCropWindowHandler.getMaxCropHeight()) / 2;
            rect.top += adj;
            rect.bottom -= adj;
        }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mLatifiCropWindowHandler.showGuidelines()) {
            // Determines whether guidelines should be drawn or not
            if (mGuidelines == LatifiCropImageView.Guidelines.ON) {
                drawGuidelines(canvas);
            } else if (mGuidelines == LatifiCropImageView.Guidelines.ON_TOUCH && mMoveHandler != null) {
    ...
    private void drawBackground(Canvas canvas) {
        ...
        RectF rect = mLatifiCropWindowHandler.getRect();
        ...
        if (mCropShape == LatifiCropImageView.CropShape.RECTANGLE) {
            ...
            mPath.reset();
            if (Build.VERSION.SDK_INT <= 17 && mCropShape == LatifiCropImageView.CropShape.OVAL) {
    ...
    private void drawCorners(Canvas canvas) {
        if (mBorderCornerPaint != null) {
            ...
            float w =
                    cornerWidth / 2
                            + (mCropShape == LatifiCropImageView.CropShape.RECTANGLE ? mBorderCornerOffset : 0);
    ...
    private void onActionDown(float x, float y) {
        mMoveHandler = mLatifiCropWindowHandler.getMoveHandler(x, y, mTouchRadius, mCropShape);
    ...
    private class ScaleListener extends ScaleGestureDetector.SimpleOnScaleGestureListener {
        ...
        public boolean onScale(ScaleGestureDetector detector) {
            ...
            if (newLeft < newRight
                    && newTop <= newBottom
                    && newLeft >= 0
                    && newRight <= mLatifiCropWindowHandler.getMaxCropWidth()
                    && newTop >= 0
                    && newBottom <= mLatifiCropWindowHandler.getMaxCropHeight()) {
