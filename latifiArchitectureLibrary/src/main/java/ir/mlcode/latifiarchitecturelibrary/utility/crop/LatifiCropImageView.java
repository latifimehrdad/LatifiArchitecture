...
public class LatifiCropImageView extends FrameLayout {
    ...
    private LatifiCropImageAnimation mAnimation;
    ...
    public LatifiCropImageView(Context context) {
        this(context, null);
    }

    public LatifiCropImageView(Context context, AttributeSet attrs) {
        super(context, attrs);

        LatifiCropImageOptions options = null;
        Intent intent = context instanceof Activity ? ((Activity) context).getIntent() : null;
        if (intent != null) {
            Bundle bundle = intent.getBundleExtra(LatifiCropImage.CROP_IMAGE_EXTRA_BUNDLE);
            if (bundle != null) {
                options = bundle.getParcelable(LatifiCropImage.CROP_IMAGE_EXTRA_OPTIONS);
            }
        }

        if (options == null) {

            options = new LatifiCropImageOptions();

            if (attrs != null) {
                TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.LatifiCropImageView, 0, 0);
                try {
                    options.fixAspectRatio =
                            ta.getBoolean(R.styleable.LatifiCropImageView_cropFixAspectRatio, options.fixAspectRatio);
                    options.aspectRatioX =
                            ta.getInteger(R.styleable.LatifiCropImageView_cropAspectRatioX, options.aspectRatioX);
                    options.aspectRatioY =
                            ta.getInteger(R.styleable.LatifiCropImageView_cropAspectRatioY, options.aspectRatioY);
                    options.scaleType =
                            ScaleType.values()[
                                    ta.getInt(R.styleable.LatifiCropImageView_cropScaleType, options.scaleType.ordinal())];
                    options.autoZoomEnabled =
                            ta.getBoolean(R.styleable.LatifiCropImageView_cropAutoZoomEnabled, options.autoZoomEnabled);
                    options.multiTouchEnabled =
                            ta.getBoolean(
                                    R.styleable.LatifiCropImageView_cropMultiTouchEnabled, options.multiTouchEnabled);
                    options.maxZoom = ta.getInteger(R.styleable.LatifiCropImageView_cropMaxZoom, options.maxZoom);
                    options.cropShape =
                            CropShape.values()[
                                    ta.getInt(R.styleable.LatifiCropImageView_cropShape, options.cropShape.ordinal())];
                    options.guidelines =
                            Guidelines.values()[
                                    ta.getInt(
                                            R.styleable.LatifiCropImageView_cropGuidelines, options.guidelines.ordinal())];
                    options.snapRadius =
                            ta.getDimension(R.styleable.LatifiCropImageView_cropSnapRadius, options.snapRadius);
                    options.touchRadius =
                            ta.getDimension(R.styleable.LatifiCropImageView_cropTouchRadius, options.touchRadius);
                    options.initialCropWindowPaddingRatio =
                            ta.getFloat(
                                    R.styleable.LatifiCropImageView_cropInitialCropWindowPaddingRatio,
                                    options.initialCropWindowPaddingRatio);
                    options.borderLineThickness =
                            ta.getDimension(
                                    R.styleable.LatifiCropImageView_cropBorderLineThickness, options.borderLineThickness);
                    options.borderLineColor =
                            ta.getInteger(R.styleable.LatifiCropImageView_cropBorderLineColor, options.borderLineColor);
                    options.borderCornerThickness =
                            ta.getDimension(
                                    R.styleable.LatifiCropImageView_cropBorderCornerThickness,
                                    options.borderCornerThickness);
                    options.borderCornerOffset =
                            ta.getDimension(
                                    R.styleable.LatifiCropImageView_cropBorderCornerOffset, options.borderCornerOffset);
                    options.borderCornerLength =
                            ta.getDimension(
                                    R.styleable.LatifiCropImageView_cropBorderCornerLength, options.borderCornerLength);
                    options.borderCornerColor =
                            ta.getInteger(
                                    R.styleable.LatifiCropImageView_cropBorderCornerColor, options.borderCornerColor);
                    options.guidelinesThickness =
                            ta.getDimension(
                                    R.styleable.LatifiCropImageView_cropGuidelinesThickness, options.guidelinesThickness);
                    options.guidelinesColor =
                            ta.getInteger(R.styleable.LatifiCropImageView_cropGuidelinesColor, options.guidelinesColor);
                    options.backgroundColor =
                            ta.getInteger(R.styleable.LatifiCropImageView_cropBackgroundColor, options.backgroundColor);
                    options.showCropOverlay =
                            ta.getBoolean(R.styleable.LatifiCropImageView_cropShowCropOverlay, mShowCropOverlay);
                    options.showProgressBar =
                            ta.getBoolean(R.styleable.LatifiCropImageView_cropShowProgressBar, mShowProgressBar);
                    options.borderCornerThickness =
                            ta.getDimension(
                                    R.styleable.LatifiCropImageView_cropBorderCornerThickness,
                                    options.borderCornerThickness);
                    options.minCropWindowWidth =
                            (int)
                                    ta.getDimension(
                                            R.styleable.LatifiCropImageView_cropMinCropWindowWidth, options.minCropWindowWidth);
                    options.minCropWindowHeight =
                            (int)
                                    ta.getDimension(
                                            R.styleable.LatifiCropImageView_cropMinCropWindowHeight,
                                            options.minCropWindowHeight);
                    options.minCropResultWidth =
                            (int)
                                    ta.getFloat(
                                            R.styleable.LatifiCropImageView_cropMinCropResultWidthPX,
                                            options.minCropResultWidth);
                    options.minCropResultHeight =
                            (int)
                                    ta.getFloat(
                                            R.styleable.LatifiCropImageView_cropMinCropResultHeightPX,
                                            options.minCropResultHeight);
                    options.maxCropResultWidth =
                            (int)
                                    ta.getFloat(
                                            R.styleable.LatifiCropImageView_cropMaxCropResultWidthPX,
                                            options.maxCropResultWidth);
                    options.maxCropResultHeight =
                            (int)
                                    ta.getFloat(
                                            R.styleable.LatifiCropImageView_cropMaxCropResultHeightPX,
                                            options.maxCropResultHeight);
                    options.flipHorizontally =
                            ta.getBoolean(
                                    R.styleable.LatifiCropImageView_cropFlipHorizontally, options.flipHorizontally);
                    options.flipVertically =
                            ta.getBoolean(R.styleable.LatifiCropImageView_cropFlipHorizontally, options.flipVertically);

                    mSaveBitmapToInstanceState =
                            ta.getBoolean(
                                    R.styleable.LatifiCropImageView_cropSaveBitmapToInstanceState,
                                    mSaveBitmapToInstanceState);

                    // if aspect ratio is set then set fixed to true
                    if (ta.hasValue(R.styleable.LatifiCropImageView_cropAspectRatioX)
                            && ta.hasValue(R.styleable.LatifiCropImageView_cropAspectRatioX)
                            && !ta.hasValue(R.styleable.LatifiCropImageView_cropFixAspectRatio)) {
        ...
        mLatifiCropOverlayView = v.findViewById(R.id.CropOverlayView);
        mLatifiCropOverlayView.setCropWindowChangeListener(
                new LatifiCropOverlayView.CropWindowChangeListener() {
                    @Override
        ...
        mLatifiCropOverlayView.setInitialAttributeValues(options);
    ...
    public void setScaleType(ScaleType scaleType) {
        if (scaleType != mScaleType) {
            ...
            mLatifiCropOverlayView.resetCropOverlayView();
            requestLayout();
        }
    }
    ...
    public CropShape getCropShape() {
        return mLatifiCropOverlayView.getCropShape();
    }
    ...
    public void setCropShape(CropShape cropShape) {
        mLatifiCropOverlayView.setCropShape(cropShape);
    }
    ...
    public void setMultiTouchEnabled(boolean multiTouchEnabled) {
        if (mLatifiCropOverlayView.setMultiTouchEnabled(multiTouchEnabled)) {
            handleCropWindowChanged(false, false);
            mLatifiCropOverlayView.invalidate();
        }
    }
    ...
    public void setMinCropResultSize(int minCropResultWidth, int minCropResultHeight) {
        mLatifiCropOverlayView.setMinCropResultSize(minCropResultWidth, minCropResultHeight);
    }
    ...
    public void setMaxCropResultSize(int maxCropResultWidth, int maxCropResultHeight) {
        mLatifiCropOverlayView.setMaxCropResultSize(maxCropResultWidth, maxCropResultHeight);
    }
    ...
    public boolean isFixAspectRatio() {
        return mLatifiCropOverlayView.isFixAspectRatio();
    }
    ...
    public void setFixedAspectRatio(boolean fixAspectRatio) {
        mLatifiCropOverlayView.setFixedAspectRatio(fixAspectRatio);
    }
    ...
    public Guidelines getGuidelines() {
        return mLatifiCropOverlayView.getGuidelines();
    }
    ...
    public void setGuidelines(Guidelines guidelines) {
        mLatifiCropOverlayView.setGuidelines(guidelines);
    }

    /** both the X and Y values of the aspectRatio. */
    public Pair<Integer, Integer> getAspectRatio() {
        return new Pair<>(mLatifiCropOverlayView.getAspectRatioX(), mLatifiCropOverlayView.getAspectRatioY());
    }
    ...
    public void setAspectRatio(int aspectRatioX, int aspectRatioY) {
        mLatifiCropOverlayView.setAspectRatioX(aspectRatioX);
        mLatifiCropOverlayView.setAspectRatioY(aspectRatioY);
        setFixedAspectRatio(true);
    }

    /** Clears set aspect ratio values and sets fixed aspect ratio to FALSE. */
    public void clearAspectRatio() {
        mLatifiCropOverlayView.setAspectRatioX(1);
        mLatifiCropOverlayView.setAspectRatioY(1);
    ...
    public void setSnapRadius(float snapRadius) {
        if (snapRadius >= 0) {
            mLatifiCropOverlayView.setSnapRadius(snapRadius);
        }
    }
    ...
    public Rect getCropRect() {
        ...
        return BitmapUtils.getRectFromPoints(
                ...
                orgWidth,
                orgHeight,
                mLatifiCropOverlayView.isFixAspectRatio(),
                mLatifiCropOverlayView.getAspectRatioX(),
                mLatifiCropOverlayView.getAspectRatioY());
    }
    ...
    public RectF getCropWindowRect() {
        if (mLatifiCropOverlayView == null) {
            return null;
        }
        return mLatifiCropOverlayView.getCropWindowRect();
    }
    ...
    public float[] getCropPoints() {
        ...
        RectF cropWindowRect = mLatifiCropOverlayView.getCropWindowRect();
    ...
    public void setCropRect(Rect rect) {
        mLatifiCropOverlayView.setInitialCropWindowRect(rect);
    }
    ...
    public void resetCropRect() {
        ...
        mLatifiCropOverlayView.resetCropWindowRect();
    }
    ...
    public void setImageBitmap(Bitmap bitmap) {
        mLatifiCropOverlayView.setInitialCropWindowRect(null);
    ...
    public void rotateImage(int degrees) {
        if (mBitmap != null) {
            ...
            BitmapUtils.RECT.set(mLatifiCropOverlayView.getCropWindowRect());
            ...
            mLatifiCropOverlayView.resetCropOverlayView();
            mLatifiCropOverlayView.setCropWindowRect(BitmapUtils.RECT);
            applyImageMatrix(getWidth(), getHeight(), true, false);
            handleCropWindowChanged(false, false);

            // make sure the crop window rectangle is within the cropping image bounds after all the
            // changes
            mLatifiCropOverlayView.fixCurrentCropWindowRect();
        }
    }
    ...
    public Parcelable onSaveInstanceState() {
        ...
        bundle.putParcelable("INITIAL_CROP_RECT", mLatifiCropOverlayView.getInitialCropWindowRect());
        ...
        bundle.putString("CROP_SHAPE", mLatifiCropOverlayView.getCropShape().name());
        ...
        if (state instanceof Bundle) {
            ...
            if (mBitmapLoadingWorkerTask == null
                ...
                if (initialCropRect != null
                    ...
                    mLatifiCropOverlayView.setInitialCropWindowRect(initialCropRect);
                }
                ...
                mLatifiCropOverlayView.setCropShape(CropShape.valueOf(bundle.getString("CROP_SHAPE")));
        ...
        if (mLayoutWidth > 0 && mLayoutHeight > 0) {
            ...
            if (mBitmap != null) {
                ...
                if (mRestoreCropWindowRect != null) {
                    ...
                    mLatifiCropOverlayView.setCropWindowRect(mRestoreCropWindowRect);
                    handleCropWindowChanged(false, false);
                    mLatifiCropOverlayView.fixCurrentCropWindowRect();
                    mRestoreCropWindowRect = null;
    ...
    private void handleCropWindowChanged(boolean inProgress, boolean animate) {
        ...
        if (mBitmap != null && width > 0 && height > 0) {
            ...
            RectF cropRect = mLatifiCropOverlayView.getCropWindowRect();
            if (inProgress) {
                if (cropRect.left < 0
                        || cropRect.top < 0
                ...
                if (newZoom > 0 && newZoom != mZoom) {
                    if (animate) {
                        if (mAnimation == null) {
                            ...
                            mAnimation = new LatifiCropImageAnimation(mImageView, mLatifiCropOverlayView);
                        }
    ...
    private void applyImageMatrix(float width, float height, boolean center, boolean animate) {
        if (mBitmap != null && width > 0 && height > 0) {
            ...
            mLatifiCropOverlayView.setCropWindowRect(cropRect);
            mapImagePointsByImageMatrix();
            mLatifiCropOverlayView.invalidate();
    ...
    private void setCropOverlayVisibility() {
        if (mLatifiCropOverlayView != null) {
            mLatifiCropOverlayView.setVisibility(mShowCropOverlay && mBitmap != null ? VISIBLE : INVISIBLE);
        }
    }
    ...
    private void updateImageBounds(boolean clear) {
        if (mBitmap != null && !clear) {
            ...
            mLatifiCropOverlayView.setCropWindowLimits(
        ...
        mLatifiCropOverlayView.setBounds(clear ? null : mImagePoints, getWidth(), getHeight());
    }
    // endregion
    ...
    public interface OnSetImageUriCompleteListener {
         ...
         */
        void onSetImageUriComplete(LatifiCropImageView view, Uri uri, Exception error);
    }
    // endregion
    ...
    public interface OnCropImageCompleteListener {
         ...
         */
        void onCropImageComplete(LatifiCropImageView view, CropResult result);
    }
    // endregion
