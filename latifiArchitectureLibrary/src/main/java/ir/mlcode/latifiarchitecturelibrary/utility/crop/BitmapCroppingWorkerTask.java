...
final class BitmapCroppingWorkerTask
    ...
    private final WeakReference<LatifiCropImageView> mCropImageViewReference;
    ...
    private final LatifiCropImageView.RequestSizeOptions mReqSizeOptions;
    ...
    BitmapCroppingWorkerTask(
            LatifiCropImageView latifiCropImageView,
            Bitmap bitmap,
            float[] cropPoints,
            int degreesRotated,
            ...
            LatifiCropImageView.RequestSizeOptions options,
            Uri saveUri,
            Bitmap.CompressFormat saveCompressFormat,
            int saveCompressQuality) {

        mCropImageViewReference = new WeakReference<>(latifiCropImageView);
        mContext = latifiCropImageView.getContext();
        mBitmap = bitmap;
    ...
    protected void onPostExecute(Result result) {
        if (result != null) {
            ...
            if (!isCancelled()) {
                LatifiCropImageView latifiCropImageView = mCropImageViewReference.get();
                if (latifiCropImageView != null) {
                    completeCalled = true;
                    latifiCropImageView.onImageCroppingAsyncComplete(result);
                }
            }
