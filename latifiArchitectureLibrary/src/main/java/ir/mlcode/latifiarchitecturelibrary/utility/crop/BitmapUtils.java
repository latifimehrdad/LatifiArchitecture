...
final class BitmapUtils {
    ...
    static Bitmap resizeBitmap(
            Bitmap bitmap, int reqWidth, int reqHeight, LatifiCropImageView.RequestSizeOptions options) {
        try {
            if (reqWidth > 0
                    && reqHeight > 0
                    && (options == LatifiCropImageView.RequestSizeOptions.RESIZE_FIT
                    || options == LatifiCropImageView.RequestSizeOptions.RESIZE_INSIDE
                    || options == LatifiCropImageView.RequestSizeOptions.RESIZE_EXACT)) {

                Bitmap resized = null;
                if (options == LatifiCropImageView.RequestSizeOptions.RESIZE_EXACT) {
                    resized = Bitmap.createScaledBitmap(bitmap, reqWidth, reqHeight, false);
                } else {
                    int width = bitmap.getWidth();
                    int height = bitmap.getHeight();
                    float scale = Math.max(width / (float) reqWidth, height / (float) reqHeight);
                    if (scale > 1 || options == LatifiCropImageView.RequestSizeOptions.RESIZE_FIT) {
                        resized =
