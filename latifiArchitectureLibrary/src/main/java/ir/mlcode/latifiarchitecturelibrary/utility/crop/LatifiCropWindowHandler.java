...
final class LatifiCropWindowHandler {
     ...
     */
    public LatifiCropWindowMoveHandler getMoveHandler(
            float x, float y, float targetRadius, LatifiCropImageView.CropShape cropShape) {
        LatifiCropWindowMoveHandler.Type type =
                cropShape == LatifiCropImageView.CropShape.OVAL
                        ? getOvalPressedMoveType(x, y)
                        : getRectanglePressedMoveType(x, y, targetRadius);
        return type != null ? new LatifiCropWindowMoveHandler(type, this, x, y) : null;
    }
     ...
     * @return the Handle that was pressed; null if no Handle was pressed
     */
    private LatifiCropWindowMoveHandler.Type getRectanglePressedMoveType(
            float x, float y, float targetRadius) {
        LatifiCropWindowMoveHandler.Type moveType = null;

        // Note: corner-handles take precedence, then side-handles, then center.
        if (LatifiCropWindowHandler.isInCornerTargetZone(x, y, mEdges.left, mEdges.top, targetRadius)) {
            moveType = LatifiCropWindowMoveHandler.Type.TOP_LEFT;
        } else if (LatifiCropWindowHandler.isInCornerTargetZone(
                x, y, mEdges.right, mEdges.top, targetRadius)) {
            moveType = LatifiCropWindowMoveHandler.Type.TOP_RIGHT;
        } else if (LatifiCropWindowHandler.isInCornerTargetZone(
                x, y, mEdges.left, mEdges.bottom, targetRadius)) {
            moveType = LatifiCropWindowMoveHandler.Type.BOTTOM_LEFT;
        } else if (LatifiCropWindowHandler.isInCornerTargetZone(
                x, y, mEdges.right, mEdges.bottom, targetRadius)) {
            moveType = LatifiCropWindowMoveHandler.Type.BOTTOM_RIGHT;
        } else if (LatifiCropWindowHandler.isInCenterTargetZone(
                x, y, mEdges.left, mEdges.top, mEdges.right, mEdges.bottom)
                && focusCenter()) {
            moveType = LatifiCropWindowMoveHandler.Type.CENTER;
        } else if (LatifiCropWindowHandler.isInHorizontalTargetZone(
                x, y, mEdges.left, mEdges.right, mEdges.top, targetRadius)) {
            moveType = LatifiCropWindowMoveHandler.Type.TOP;
        } else if (LatifiCropWindowHandler.isInHorizontalTargetZone(
                x, y, mEdges.left, mEdges.right, mEdges.bottom, targetRadius)) {
            moveType = LatifiCropWindowMoveHandler.Type.BOTTOM;
        } else if (LatifiCropWindowHandler.isInVerticalTargetZone(
                x, y, mEdges.left, mEdges.top, mEdges.bottom, targetRadius)) {
            moveType = LatifiCropWindowMoveHandler.Type.LEFT;
        } else if (LatifiCropWindowHandler.isInVerticalTargetZone(
                x, y, mEdges.right, mEdges.top, mEdges.bottom, targetRadius)) {
            moveType = LatifiCropWindowMoveHandler.Type.RIGHT;
        } else if (LatifiCropWindowHandler.isInCenterTargetZone(
                x, y, mEdges.left, mEdges.top, mEdges.right, mEdges.bottom)
                && !focusCenter()) {
            moveType = LatifiCropWindowMoveHandler.Type.CENTER;
     ...
     * @return the Handle that was pressed; null if no Handle was pressed
     */
    private LatifiCropWindowMoveHandler.Type getOvalPressedMoveType(float x, float y) {
    ...
    */
        ...
        LatifiCropWindowMoveHandler.Type moveType;
        if (x < leftCenter) {
            if (y < topCenter) {
                moveType = LatifiCropWindowMoveHandler.Type.TOP_LEFT;
