...
final class BitmapLoadingWorkerTask extends AsyncTask<Void, Void, BitmapLoadingWorkerTask.Result> {
    ...
    public BitmapLoadingWorkerTask(LatifiCropImageView latifiCropImageView, Uri uri) {
        mUri = uri;
        mCropImageViewReference = new WeakReference<>(latifiCropImageView);

        mContext = latifiCropImageView.getContext();

        DisplayMetrics metrics = latifiCropImageView.getResources().getDisplayMetrics();
        ...
        if (result != null) {
            ...
            if (!isCancelled()) {
                ...
                if (latifiCropImageView != null) {
                    completeCalled = true;
                    latifiCropImageView.onSetImageUriAsyncComplete(result);
                }
            }
